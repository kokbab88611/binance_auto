    ...
    def close_position(self, current_price):
        if self.position == "long" and self.position_status:
            if current_price >= self.price_profit or current_price <= self.price_stoploss:
                ...
                self.save_result(f"Closed long position at {current_price} with {result}")
                self.position = None
        elif self.position == "short" and self.position_status:
            if current_price <= self.price_profit or current_price >= self.price_stoploss:
                self.position_status = False
                result = "profit" if current_price <= self.price_profit else "loss"
                self.save_result(f"Closed short position at {current_price} with {result}")
                self.position = None
...
class BinanceTrade:
    ...
    def order(self, symbol, side, reduceOnly, quantity):
        print(f"Simulated order: {side} {quantity} of {symbol}")
