...
class Strategy:
    ...
    def check_trade_signal(df_5m, df_15m, df_1h, binancetrade):
        ...
        # Trade signal
        if long_condition:
            quantity = binancetrade.market_open_position(side="BUY", position_side="LONG")
            ...
            quantity = binancetrade.market_open_position(side="SELL", position_side="SHORT")
            binancetrade.order_sl_tp(current_price, "long" , atr_15m, quantity, atr_multiplier_tp=2, atr_multiplier_sl=1.75)
    ...
    def box_trend_strategy(df_5m, df_15m, df_30m, binancetrade):
        ...
        if (current_price > resistance_mean + atr_5m) and (current_price > recent_high_30):
            #롱 돌파
            Strategy.box_breakout('long', df_5m, atr_15m, current_price, binancetrade)
        elif (current_price < support_mean - atr_5m) and (current_price < recent_low_30):
            #숏 돌파
            Strategy.box_breakout('short', df_5m, atr_15m, current_price, binancetrade)
    ...
    @staticmethod
    def box_breakout(trend, df_5m, atr_15m, current_price, binancetrade):
