...
class DataCollector:
    def __init__(self):
        ...
        self.supertrend_history_df = pd.DataFrame(columns=['timestamp', 'supertrend'])
    ...
    def supertrend(self, df, atr_period=10, multiplier=3):
        high = df['high']
        low = df['low']
        close = df['close']
    ...
    def update_supertrend_history(self, atr_period=10, multiplier=3):
        for i in range(len(self.main_df) - 30, len(self.main_df)):
            sub_df = self.main_df.iloc[:i+1]
            supertrend_signal = self.supertrend(sub_df, atr_period, multiplier).iloc[-1]
            timestamp = sub_df['openTime'].iloc[-1]
            new_entry = pd.DataFrame({'timestamp': [timestamp], 'supertrend': [supertrend_signal]})
            self.supertrend_history_df = pd.concat([self.supertrend_history_df, new_entry], ignore_index=True)

            if len(self.supertrend_history_df) > 50:
                self.supertrend_history_df = self.supertrend_history_df.iloc[-50:]

            print(f"Supertrend history updated: {self.supertrend_history_df.tail()}")
    ...
    def decision(self, current_price):
        ...
        atr = self.ATR()
        atr_last = atr.iloc[-1]
        supertrend_signal = self.supertrend(atr, current_price)
